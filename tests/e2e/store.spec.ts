...
test("flujo e2e mixto: catalogo, carrito, checkout, pago dev y entrega digital", async ({
  ...
  await page.getByRole("link", { name: /Juego digital de ejemplo/i }).click();
  ...
  await page.getByLabel("Telefono").fill("1122334455");
  await page.getByLabel("Direccion").fill("Av. Siempre Viva 123");
  ...
  await expect(page.getByRole("heading", { name: "Pedido creado" })).toBeVisible();
  ...
  await page.getByPlaceholder(/Contenido o referencia segura/i).fill("Referencia test");
